import { ref } from "vue";
import { SetupVendorBusinessParams } from "@/features/shared/types/types";
...
} from "@/features/vendors/services/VendorsRestDaysService";
import {
  VendorsPortfolioTag,
...
export function useVendorBusinessSetup() {
  ...
  async function setupVendorBusiness(params: SetupVendorBusinessParams) {
    ...
    // 2. Vendor regions
    if (regions.length) {
      const { error: regionErr } = await VendorRegionService.createBatch(
        updated.regions,
      );
      if (regionErr) {
      ...
      stepsDone.push(async () => {
        await VendorRegionService.deleteByVendorId(vendorId!);
      });
    }

    // 3. Niche custom info (using serviceTypeId and nicheInfo)
    const { data: serviceType, error: stErr } =
      await ServiceTypesService.getById(serviceTypeId);
    ...
    if (NICHES.includes(serviceTypeName) && nicheInfo) {
      ...
      if (customService) {
        ...
        const { error: customErr } = await customService.create({
          ...nichePayload,
          vendor_id: vendorId,
        });
        if (customErr) {
        ...
        stepsDone.push(async () => {
          await customService.deleteByVendorId(vendorId!);
        });
      }
    }
    ...
    // 4. Packages
    if (packages.length) {
      const { error: pkgErr } = await VendorPackageService.createBatch(
        updated.packages,
      );
      if (pkgErr) {
      ...
      stepsDone.push(async () => {
        await VendorPackageService.deleteByVendorId(vendorId!);
      });
    }

    // 5. Promotions
    if (promotions.length) {
      const { error: promoErr } = await VendorPromotionService.createBatch(
        updated.promotions,
      );
      if (promoErr) {
      ...
      stepsDone.push(async () => {
        await VendorPromotionService.deleteByVendorId(vendorId!);
      });
    }

    // 6. Rest days
    if (restDays.length) {
      const { error: restErr } = await VendorsRestDaysService.createBatch(
        updated.restDays,
      );
      if (restErr) {
      ...
      stepsDone.push(async () => {
        await VendorsRestDaysService.deleteByVendorId(vendorId!);
      });
    }

    // 7. Portfolio tags
    if (portfolioTags.length) {
      const { error: tagErr } = await VendorsPortfolioTagsService.createBatch(
        updated.portfolioTags,
      );
      if (tagErr) {
      ...
      stepsDone.push(async () => {
        await VendorsPortfolioTagsService.deleteByVendorId(vendorId!);
      });
    }
...
function updateVendorIdForAll(
  ...
  return {
    regions: regions.map(({ id, created_at, ...rest }) => ({
      ...rest,
      vendor_id: vendorId,
    })),
    ...
    promotions: promotions.map(({ id, created_at, ...rest }) => ({
      ...rest,
      vendor_id: vendorId,
    })),
    restDays: restDays.map(({ id, created_at, ...rest }) => ({
      ...rest,
      vendor_id: vendorId,
    })),
    portfolioTags: portfolioTags.map(({ id, ...rest }) => ({
      ...rest,
      vendor_id: vendorId,
    })),
  };
